...
export function CTA() {
  return (
    <section id="contact" className="relative overflow-hidden bg-dark py-24 sm:py-28">
      <motion.div
        className="pointer-events-none absolute -left-24 top-1/2 h-72 w-72 -translate-y-1/2 rounded-full bg-indigo/40 blur-3xl"
        animate={{ x: [0, 80, 0], y: [0, -30, 0] }}
        transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="pointer-events-none absolute -right-16 top-10 h-80 w-80 rounded-full bg-cyan/30 blur-3xl"
        animate={{ x: [0, -60, 0], y: [0, 40, 0] }}
        transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="pointer-events-none absolute bottom-0 left-1/3 h-64 w-64 rounded-full bg-purple/35 blur-3xl"
      ...
      <div className="relative mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
        <MotionDiv>
          ...
          <h2 className="mt-4 text-3xl font-black tracking-tight text-white sm:text-5xl">
            Need printer, PC, or solar help?{" "}
            <span className="bg-gradient-to-r from-indigo via-purple to-cyan bg-clip-text text-transparent">
              We&apos;re ready.
            </span>
          </h2>
          <p className="mx-auto mt-5 max-w-2xl text-base leading-relaxed text-zinc-400 sm:text-lg">
            Tell us whether you need printer repair, PC &amp; laptop repair, or solar installation.
            We&apos;ll respond with a clear plan, timeline, and quote — usually within the hour.
          </p>
        </MotionDiv>
          ...
          {PEOPLE.map((item) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-0.5 text-xs font-medium uppercase tracking-wider text-cyan">
                  {item.note}
                </p>
                <a
                  href={person.tel}
                  className="mt-4 inline-flex min-h-11 w-full items-center justify-center rounded-full bg-gradient-to-r from-indigo to-purple px-5 text-sm font-semibold text-white shadow-lg shadow-indigo/30 transition-all duration-200 hover:scale-[1.02]"
                >
